...
/* ====== Theme Manager (يدعم: default, light, pink, black, highcontrast, lightblue) ====== */
const THEME_KEY = 'grsim.theme';
const themeOrder = ['default','light','pink','black','highcontrast','lightblue'];

function applyTheme(name){
  const root = document.documentElement;
  root.classList.remove('light','pink','black','highcontrast','lightblue'); // امسح القديم
  if (name && name !== 'default') root.classList.add(name);                 // أضف الجديد
...
function cycleTheme(){
  const cur = localStorage.getItem(THEME_KEY) || 'default';
...
bindUI();
startAutoReorbit();
initThemeUI();         // ← تهيئة الثيمات الجديدة
requestAnimationFrame(loop);
